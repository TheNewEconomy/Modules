...
public class RealEconomy extends Converter {
  private File dataDirectory = new File(TNECore.directory(), "../RealEconomy/userdata");
  ...
  public File dataFolder() {
    return new File(TNECore.directory(), "../RealEconomy/config.yml");
  }
  ...
  public void yaml() throws InvalidDatabaseImport {
    ...
    for(File accountFile : dataDirectory.listFiles()) {
      ...
      ConversionModule.convertedAdd(accountFile.getName().replace(".yml", ""), TNECore.server().defaultRegion(TNECore.eco().region().getMode()), currency, new BigDecimal(money));
    }
  }
}