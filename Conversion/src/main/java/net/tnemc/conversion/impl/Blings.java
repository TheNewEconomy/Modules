...
public class Blings extends Converter {
  private File configFile = new File(TNECore.directory(), "../Blings/players.yml");
  private YamlFile config = YamlFile.loadConfiguration(configFile, true);

  public Blings() throws IOException { }
  ...
  public File dataFolder() {
    return new File(TNECore.directory(), "../Blings/config.yml");
  }
  ...
  public void yaml() throws InvalidDatabaseImport {
    ...
    if(accountSection != null) {
      ...
      for(String uuid : accounts) {
        ...
        ConversionModule.convertedAdd(uuid, TNECore.server().defaultRegion(TNECore.eco().region().getMode()), cur.getIdentifier(), new BigDecimal(config.getString("Money." + uuid)));
      }
    }
  }
}
