...
public class Meller extends Converter {
  private File configFile = new File(TNECore.directory(), "../Meller/economy.yml");
  ...
  public File dataFolder() {
    return new File(TNECore.directory(), "../Meller/config.yml");
  }
  ...
  public void yaml() throws InvalidDatabaseImport {
    ...
    if(accountSection != null) {
      ...
      for(String uuid : accounts) {
        ...
        ConversionModule.convertedAdd(uuid, TNECore.server().defaultRegion(TNECore.eco().region().getMode()), cur.getIdentifier(), new BigDecimal(config.getString("accounts." + uuid + ".balance")));
      }
    }
  }
}
