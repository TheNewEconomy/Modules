...
public class BasicEconomy extends Converter {
  private final File configFile = new File(TNECore.directory(), "../BasicEconomy/config.yml");
  private final YamlFile config = YamlFile.loadConfiguration(configFile, true);

  public BasicEconomy() throws IOException {
  }
  ...
  public File dataFolder() {
    return new File(TNECore.directory(), "../BasicEconomy/config.yml");
  }
  ...
  public void yaml() throws InvalidDatabaseImport {
    ...
    if(accountSection != null) {
      ...
      for(String uuid : accounts) {
        ...
        ConversionModule.convertedAdd(uuid, TNECore.server().defaultRegion(TNECore.eco().region().getMode()), cur.getIdentifier(), new BigDecimal(config.getString("balances." + uuid)));
      }
    }
  }
}
