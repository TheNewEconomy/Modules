 ...
 * @since 0.1.0.0
 */
public class SpongeConvert implements OrphanCommand {

    @Subcommand({"convert"})
    @Usage("/tne convert")
    @Description("Used to convert from other economy plugins.")
    @CommandPermission("tne.admin.convert")
    public void convert(SpongeCommandActor sender, Converter converter) {
        ConvertCommand.handle(new SpongeCMDSource(sender), converter);
    }
}